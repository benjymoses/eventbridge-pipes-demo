...
export class EbPipesDemoStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const catchAllLogRule = new Rule(this, 'CatchAllLogRule', {
      description: 'Used to capture all events on the custom EventBridge bus for demo purposes',
      ruleName: 'CatchAllLogRule',
      eventBus: eventBus,
      eventPattern: {
        source: Match.prefix(''),
      },
      targets: [new targets.CloudWatchLogGroup(new LogGroup(this, 'CatchAllLogGroup', {
    ...
    const fraudRule = new Rule(this, 'HighFraudRule', {
      description: 'All orders with a Fraud Score higher than 80 go to fraud team email',
      eventBus: eventBus,
      eventPattern: { 
    ...
    const enricherWorkflow = new stepfunctions.StateMachine(this, 'EnricherWorkflowForFraud', {
      //stateMachineName: 'enricherWorkflow',
      ...
      definition: stepfunctions.Chain.start(
        new stepfunctions.Map(this, 'Map over events in batch', {
          maxConcurrency: 5,
          //itemsPath: stepfunctions.JsonPath.stringAt('$')
        })
        .iterator(
          new stepfunctions.Pass(this, 'Deserialise DynamoDB', {
            parameters: {
              eventData: {
                ...
                approxDateTime: stepfunctions.JsonPath.numberAt('$.dynamodb.ApproximateCreationDateTime')
              },
              orderData: {
                orderNumber: stepfunctions.JsonPath.stringAt('$.dynamodb.NewImage.PK.S'),
                customerName: stepfunctions.JsonPath.stringAt('$.dynamodb.NewImage.name.S'),
                orderValue: stepfunctions.JsonPath.stringToJson(stepfunctions.JsonPath.stringAt('$.dynamodb.NewImage.value.N'))
              }
            } // Parameter creates new JSON object from contents of original input payload
          })
          .next(new stepfunctions.Pass(this, 'Fraud Check', {
            resultPath: '$.fraudChecks',
            parameters: {fraudScore: stepfunctions.JsonPath.mathRandom(1,100)}
    ...
    const filterInsertsWithFields: CfnPipe.FilterCriteriaProperty = {
      filters: [{
        pattern: "{\"eventName\":[\"INSERT\"],\"dynamodb\":{\"NewImage\":{\"PK\":{\"S\":[{\"exists\":true}]},\"name\":{\"S\":[{\"exists\":true}]},\"value\":{\"N\":[{\"exists\":true}]}}}}",
      }],
    }; 

  // EventBridge Pipe to connect DynamoDB stream to EventBridge bus with in-line fraud checking enrichment by Step Functions.
    if (typeof ddbTable.tableStreamArn === "string") { // CfnPipe.source is a required Param and TS is unable to assert it won't be undefined
